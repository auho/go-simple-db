...
func CopyStructureSQL(src, dst string) string {
	return fmt.Sprintf("CREATE TABLE %s LIKE %s", QuoteIdentifier(dst), QuoteIdentifier(src))
}
...
const GetDatabaseSQL = "SELECT DATABASE()"

// GetTableColumnsSchemaSQL returns the SQL to get column schemas for a table.
const GetTableColumnsSchemaSQL = "SELECT `COLUMN_NAME` AS `name`, `DATA_TYPE` AS `field_type` " +
	"FROM `information_schema`.`COLUMNS` " +
	"WHERE `TABLE_SCHEMA` = ? AND `TABLE_NAME` = ?"

// GetTableColumnsSQL returns the SQL to get column names for a table.
const GetTableColumnsSQL = "SELECT `COLUMN_NAME` " +
	"FROM `information_schema`.`COLUMNS` " +
	"WHERE `TABLE_SCHEMA` = ? AND `TABLE_NAME` = ?"
